...
// http://nomisma.cloud.testinsights.io/app/#!/module-collection/guid/6757b7f3-d904-4db8-a0ff-309270317303
@TestModellerModule(guid = "6757b7f3-d904-4db8-a0ff-309270317303")
public class Total_countfiled_status_verififcation extends BasePage
{
	public Total_countfiled_status_verififcation (WebDriver driver)
	{
		super(driver);
	}
	...
	private By _5Elem = By.xpath("//A[contains(text(),'5')]");
	...
	private By _12Elem = By.xpath("//B[contains(text(),'12')]");
...
WebElement eles= m_Driver.findElement(By.xpath("//*[@id=\"ctl00_ctl00_ParentContent_divSubContent\"]/div[9]/div[2]/div/div[1]/div/table"));
		...
		List<WebElement> TotalRowCount = eles.findElements(By.xpath("//div[@id=\"ctl00_ctl00_ParentContent_divSubContent\"]//div/table/tbody//tr/td[13]"));
...
WebElement eles= m_Driver.findElement(By.xpath("//*[@id=\"ctl00_ctl00_ParentContent_divSubContent\"]/div[9]/div[2]/div/div[1]/div/table"));
     ...
     * @name Click  12
     */
	public void Click__12()
	{
        
		WebElement elem = getWebElement(_12Elem);

		if (elem == null) {
    		ExtentReportManager.failStepWithScreenshot(m_Driver, "Click__12", "Click__12 failed. Unable to locate object: " + _12Elem.toString());

    		TestModellerLogger.FailStepWithScreenshot(m_Driver, "Click__12", "Click__12 failed. Unable to locate object: " + _12Elem.toString());

			Assert.fail("Unable to locate object: " + _12Elem.toString());
        }

		String txt= elem.getText();
		_acount=Integer.parseInt(txt);
		
		System.out.println("total count:filed status" +_acount);
		elem.click();
          	

		ExtentReportManager.passStep(m_Driver, "Click__12");

		TestModellerLogger.PassStep(m_Driver, "Click__12");
	}
}